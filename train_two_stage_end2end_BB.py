...
from layers.modules import MultiBoxLoss_offset, MultiBoxLoss
from ssd_two_stage_end2end_BB import build_ssd
import os
import sys
import time
import torch
...
def train():
    ...
    criterion = MultiBoxLoss_offset(cfg['num_classes'], 0.5, True, 0, True, 3, 0.5,
                             False, args.cuda)
    criterion_2 = MultiBoxLoss(cfg['num_classes'], 0.5, True, 0, True, 3, 0.5,
                             False, args.cuda)
    ...
    if args.visdom:
        ...
        vis_legend = ['Loc Loss', 'Conf Loss', 'Size LP Loss', 'Offset Loss', 'Has LP Loss',
                      'Loc2 loss', 'Conf2 Loss', 'Total Loss']
    ...
    for iteration in range(args.start_iter, cfg['max_iter']):
        ...
        loc_2_data, conf_2_data, priors_2 = out[6:-1]
        targets_2 = out[-1]
        ...
        if targets_2.shape[0] > 0:
            ...
            loss_l_2, loss_c_2 = criterion_2(out_2, targets_2_list)
            loss_2 = loss_l_2 + loss_c_2
            loss = loss_1 + loss_2
        else:
            loss = loss_1
        ...
        if args.visdom:
            if targets_2.shape[0] > 0:
                ...
                update_vis_plot(iteration, loss_l.item(), loss_c.item(), loss_size_lp.item(), loss_offset.item(),
                                loss_has_lp.item(), 0, 0, iter_plot, epoch_plot, 'append')
...
def update_vis_plot(iteration, loc, conf, size_lp, offset, has_lp, loc_2, conf_2,
                    window1, window2, update_type, epoch_size=1):
    viz.line(
        X=torch.ones((1, 8)).cpu() * iteration,
        Y=torch.Tensor([loc, conf, size_lp, offset, has_lp,
                        loc_2, conf_2,
                        loc + conf + size_lp + offset + has_lp + loc_2 + conf_2]).unsqueeze(0).cpu() / epoch_size,
        win=window1,
        update=update_type
    )
    ...
    if iteration == 0:
        viz.line(
            X=torch.zeros((1, 8)).cpu(),
            Y=torch.Tensor([loc, conf, size_lp, offset, has_lp,
                            loc_2, conf_2,
                            loc + conf + size_lp + offset + has_lp + loc_2 + conf_2]).unsqueeze(0).cpu(),
            win=window2,
            update=True
        )
