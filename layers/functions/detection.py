...
from ..box_utils import decode_four_corners
    ...
    def forward(self, loc_data, conf_data, prior_data, has_lp_data, size_lp_data, offset_data):
        ...
        for i in range(num):
            ...
            for cl in range(1, self.num_classes):  # 从1开始，去掉了背景
                ...
                # ids: 大小是[number great than conf_threshold], 里面只有nms后保留的框的idx被留下来，其余全部是0
                # count: 里面只有nms后保留的框的个数
                ids, count = nms(boxes, scores, self.nms_thresh, self.top_k)
                output[i, cl, :count] = \
                    torch.cat((scores[ids[:count]].unsqueeze(1),
                               boxes[ids[:count]], scores_lp[ids[:count]].unsqueeze(1),
                               size_lp[ids[:count]], offset[ids[:count]]), 1)  # 第i张图片里第cl类第n个框的置信度+四个位置坐标
    ...
    def forward(self, loc_data, conf_data, prior_data, four_corners_data):
        ...
        for i in range(num):
            ...
            for cl in range(1, self.num_classes):  # 从1开始，去掉了背景
                ...
                output[i, cl, :count] = \
                    torch.cat((scores[ids[:count]].unsqueeze(1),
                               boxes[ids[:count]], corners[ids[:count]]), 1)  # 第i张图片里第cl类第n个框的置信度+四个位置坐标
...
class Detect_only_four_corners(Function):
    """At test time, Detect is the final layer of SSD.  Decode location preds,
    apply non-maximum suppression to location predictions based on conf
    scores and threshold to a top_k number of output predictions for both
    confidence score and locations.
    ...
    def forward(self, conf_data, prior_data, four_corners_data):
        """
        Args:
            loc_data: (tensor) Loc preds from loc layers
                Shape: [batch,num_priors,4]
            ...
            four_corners_data: (tensor) Four corners preds from four_corners layers
                Shape: [batch,num_priors,8]
        """
        num = conf_data.size(0)  # batch size
        num_priors = prior_data.size(0)
        output = torch.zeros(num, self.num_classes, self.top_k, 13)
        conf_preds = conf_data.view(num, num_priors,
                                    self.num_classes).transpose(2, 1)  # [batch,num_classes,num_priors]
        ...
        for i in range(num):
            ...
            for cl in range(1, self.num_classes):  # 从1开始，去掉了背景
                ...
                corners = decoded_corners[l_mask].view(-1, 8)  # [number great than conf_threshold, 8]
                # 这里的bbox换成由四点得到
                boxes = torch.zeros([corners.shape[0], 4])
                boxes[:, 0] = torch.min(corners[:, 0], corners[:, 6])
                boxes[:, 1] = torch.min(corners[:, 1], corners[:, 3])
                boxes[:, 2] = torch.max(corners[:, 2], corners[:, 4])
                boxes[:, 3] = torch.max(corners[:, 5], corners[:, 7])
                # idx of highest scoring and non-overlapping boxes per class
                # ids: 大小是[number great than conf_threshold], 里面只有nms后保留的框的idx被留下来，其余全部是0
                # count: 里面只有nms后保留的框的个数
                ids, count = nms(boxes, scores, self.nms_thresh, self.top_k)
