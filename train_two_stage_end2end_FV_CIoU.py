...
from layers.modules import MultiBoxLoss_offset, MultiBoxLoss_only_four_corners_with_CIoU
from ssd_two_stage_end2end_FV_CIoU import build_ssd
import os
import sys
import time
import torch
...
parser.add_argument('--obj_type', default='two_stage_end2end_with_border', choices=['two_stage_end2end_with_border'],
                    type=str, help='two_stage_end2end_with_border')
...
def train():
    ...
    criterion = MultiBoxLoss_offset(cfg['num_classes'], 0.5, True, 0, True, 3, 0.5,
                             False, args.cuda)
    criterion_2 = MultiBoxLoss_only_four_corners_with_CIoU(cfg['num_classes'], 0.5, True, 0, True, 3, 0.5,
                             False, args.cuda)
    ...
    if args.visdom:
        ...
        vis_legend = ['Loc Loss', 'Conf Loss', 'Size LP Loss', 'Offset Loss', 'Has LP Loss',
                      'Conf2 Loss', 'Four Corners2 Loss', 'Border2 loss', 'Total Loss']
    ...
    for iteration in range(args.start_iter, cfg['max_iter']):
        if args.visdom and iteration != 0 and (iteration % epoch_size == 0):
            epoch += 1
            update_vis_plot(epoch, loc_loss, conf_loss, size_lp_loss, offset_loss, has_lp_loss, conf_2_loss,
        ...
        conf_2_data, priors_2, four_corners_2_data = out[6:-1]
        targets_2 = out[-1]
        ...
        if targets_2.shape[0] > 0:
            ...
            loss_c_2, loss_four_corners_2, loss_border_2 = criterion_2(out_2, targets_2_list)
            loss_2 = loss_c_2 + loss_four_corners_2 + loss_border_2
            loss = loss_1 + loss_2
        else:
            loss = loss_1
        ...
        if args.visdom:
            if targets_2.shape[0] > 0:
                update_vis_plot(iteration, loss_l.item(), loss_c.item(), loss_size_lp.item(), loss_offset.item(),
                                loss_has_lp.item(), loss_c_2.item(), loss_four_corners_2.item(),
                                loss_border_2.item(), iter_plot, epoch_plot, 'append')
            else:
                update_vis_plot(iteration, loss_l.item(), loss_c.item(), loss_size_lp.item(), loss_offset.item(),
...
def update_vis_plot(iteration, loc, conf, size_lp, offset, has_lp, conf_2, four_corners_2, border_2,
                    window1, window2, update_type, epoch_size=1):
    viz.line(
        X=torch.ones((1, 9)).cpu() * iteration,
        Y=torch.Tensor([loc, conf, size_lp, offset, has_lp,
                        conf_2, four_corners_2, border_2,
                        loc + conf + size_lp + offset + has_lp + conf_2 + four_corners_2 + border_2]).unsqueeze(0).cpu() / epoch_size,
        win=window1,
        update=update_type
    )
    ...
    if iteration == 0:
        viz.line(
            ...
            Y=torch.Tensor([loc, conf, size_lp, offset, has_lp,
                            conf_2, four_corners_2, border_2,
                            loc + conf + size_lp + offset + has_lp + conf_2 + four_corners_2 + border_2]).unsqueeze(0).cpu(),
            win=window2,
            update=True
        )
