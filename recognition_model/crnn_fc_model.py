...
class CRNN_FC(nn.Module):
    ...
    def __init__(self, imgH, nc, nclass, isPretrain=False, leakyRelu=False):
        super(CRNN_FC, self).__init__()
        print(imgH, nc, nclass, leakyRelu)
        print("using fc all model")
        ...
        def convRelu(i, batchNormalization=False):
            ...
            nOut = nm[i]
            if i < 3:
                cnn_1.add_module('conv{0}'.format(i),
                            nn.Conv2d(nIn, nOut, ks[i], ss[i], ps[i]))
                if batchNormalization:
                    cnn_1.add_module('batchnorm{0}'.format(i), nn.BatchNorm2d(nOut))
                if leakyRelu:
                    cnn_1.add_module('relu{0}'.format(i),
                                nn.LeakyReLU(0.2, inplace=True))
                else:
                    cnn_1.add_module('relu{0}'.format(i), nn.ReLU(True))
            else:
                cnn_2.add_module('conv{0}'.format(i),
                            nn.Conv2d(nIn, nOut, ks[i], ss[i], ps[i]))
                if batchNormalization:
                    cnn_2.add_module('batchnorm{0}'.format(i), nn.BatchNorm2d(nOut))
                if leakyRelu:
                    cnn_2.add_module('relu{0}'.format(i),
                                nn.LeakyReLU(0.2, inplace=True))
                else:
                    cnn_2.add_module('relu{0}'.format(i), nn.ReLU(True))

        convRelu(0, True)
        cnn_1.add_module('pooling{0}'.format(0), nn.MaxPool2d(2, 2))  # 64x16x64
        convRelu(1, True)
        cnn_1.add_module('pooling{0}'.format(1), nn.MaxPool2d(2, 2))  # 128x8x32
        convRelu(2, True)
        convRelu(3, True)
        cnn_2.add_module('pooling{0}'.format(2), nn.MaxPool2d((2, 2), (2, 1), (0, 0)))  # 256x4x16
        convRelu(4, True)
        convRelu(5, True)
          # 512x2x16
    ...
    def forward(self, input):
        # conv features
        conv_out_1 = self.cnn_1(input)
        conv_out = self.cnn_2(conv_out_1)
        # print(conv_out_1.size())
