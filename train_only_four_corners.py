...
from layers.modules import MultiBoxLoss_only_four_corners
...
parser.add_argument('--dataset', default='CARPLATE_ONLY_FOUR_CORNERS', choices=['CARPLATE_ONLY_FOUR_CORNERS'],
                    type=str, help='CARPLATE_ONLY_FOUR_CORNERS')
...
parser.add_argument('--obj_type', default='carplate_only_four_corners', choices=['carplate_only_four_corners'],
                    type=str, help='carplate_only_four_corners')
...
def train():
    ...
    #                        weight_decay=args.weight_decay)
    criterion = MultiBoxLoss_only_four_corners(cfg['num_classes'], 0.5, True, 0, True, 3, 0.5,
                             False, args.cuda)
    ...
    if args.visdom:
        ...
        vis_legend = ['Conf Loss', 'Four Corners Loss', 'Total Loss']
    ...
    for iteration in range(args.start_iter, cfg['max_iter']):
        if args.visdom and iteration != 0 and (iteration % epoch_size == 0):
            epoch += 1
            update_vis_plot(epoch, conf_loss, four_corners_loss, epoch_plot, None,
                            'append', epoch_size)
        ...
        # backprop
        optimizer.zero_grad()
        loss_c, loss_four_corners = criterion(out, targets)
        loss = loss_c + loss_four_corners
        loss.backward()
        optimizer.step()
        t1 = time.time()
        ...
        if iteration % 100 == 0:
            log.l.info('''
                Timer: {:.5f} sec.\t LR: {}.\t Iter: {}.\t Loss_c: {:.5f}.\t Loss_four_corners: {:.5f}.\t Loss: {:.5f}.
                '''.format((t1-t0), lr, iteration, loss_c.item(), loss_four_corners.item(),
                loss_c.item() + loss_four_corners.item()))
...
def update_vis_plot(iteration, conf, four_corners, window1, window2, update_type,
                    epoch_size=1):
    viz.line(
        X=torch.ones((1, 3)).cpu() * iteration,
        Y=torch.Tensor([conf, four_corners, conf + four_corners]).unsqueeze(0).cpu() / epoch_size,
        win=window1,
        update=update_type
    )
    ...
    if iteration == 0:
        viz.line(
            ...
            Y=torch.Tensor([conf, four_corners, conf + four_corners]).unsqueeze(0).cpu(),
            win=window2,
            update=True
        )
