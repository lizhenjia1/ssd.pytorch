...
CAR_CARPLATE_OFFSET_CLASSES = (  # always index 0
    'car', )
...
class CAR_CARPLATE_OFFSETAnnotationTransform(object):
    ...
    def __init__(self, class_to_ind=None, keep_difficult=False):
        self.class_to_ind = class_to_ind or dict(
            zip(CAR_CARPLATE_OFFSET_CLASSES, range(len(CAR_CARPLATE_OFFSET_CLASSES))))
    ...
    def __call__(self, target, width, height):
        ...
        for obj in target.iter('object'):
            ...
            # 如果没有车牌,has_carplate,size和offset设置为0
            res += [bndbox]  # [xmin, ymin, xmax, ymax, has_carplate, width, height, x_offset, y_offset, label_ind]
            # img_id = target.find('filename').text[:-4]

        return res  # [[xmin, ymin, xmax, ymax, has_carplate, width, height, x_offset, y_offset, label_ind], ... ]


class CAR_CARPLATE_OFFSETDetection(data.Dataset):
    ...
    def __init__(self, root,
                 image_sets=None,
                 transform=None, target_transform=CAR_CARPLATE_OFFSETAnnotationTransform(keep_difficult=True),
