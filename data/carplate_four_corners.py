...
CARPLATE_FOUR_CORNERS_CLASSES = (  # always index 0
    'carplate', )

# note: if you used our download scripts, this should be right
CARPLATE_FOUR_CORNERS_ROOT = osp.join('/data', 'TILT/720p/carplate_only/')


class CARPLATE_FOUR_CORNERSAnnotationTransform(object):
    ...
    def __init__(self, class_to_ind=None, keep_difficult=False, object_size='all'):
        self.class_to_ind = class_to_ind or dict(
            zip(CARPLATE_FOUR_CORNERS_CLASSES, range(len(CARPLATE_FOUR_CORNERS_CLASSES))))
    ...
    def __call__(self, target, width, height):
        ...
        for obj in target.iter('object'):
            ...
            # four corners
            corners = ['x_top_left', 'y_top_left', 'x_top_right', 'y_top_right', 'x_bottom_right', 'y_bottom_right',
                       'x_bottom_left', 'y_bottom_left']
            for i, corner in enumerate(corners):
                cur_corner = int(bbox.find(corner).text) - 1
                # scale height or width
                cur_corner = cur_corner / width if i % 2 == 0 else cur_corner / height
                bndbox.append(cur_corner)
            # label -1
            label_idx = self.class_to_ind[name]
            bndbox.append(label_idx)
            res += [bndbox]  # [xmin, ymin, xmax, ymax, x_top_left, y_top_left, ..., label_ind]
            # img_id = target.find('filename').text[:-4]
        return res  # [[xmin, ymin, xmax, ymax, x_top_left, y_top_left, ..., label_ind], ... ]


class CARPLATE_FOUR_CORNERSDetection(data.Dataset):
    ...
    def __init__(self, root,
                 image_sets=None,
                 transform=None, target_transform=CARPLATE_FOUR_CORNERSAnnotationTransform(keep_difficult=True, object_size='all'),
